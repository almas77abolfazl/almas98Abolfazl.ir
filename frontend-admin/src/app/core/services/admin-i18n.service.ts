import { Injectable, signal, effect } from '@angular/core';

export type Language = 'en' | 'fa';

const TRANSLATIONS: Record<Language, Record<string, string>> = {
  en: {
    appName: 'Admin Panel',
    // Nav
    nav_dashboard: 'Dashboard',
    nav_about: 'About Me',
    nav_experiences: 'Experiences',
    nav_educations: 'Educations',
    nav_skills: 'Skills',
    nav_skill_categories: 'Skill Categories',
    nav_articles: 'Articles',
    nav_videos: 'Videos',
    nav_projects: 'Projects',
    nav_testimonials: 'Testimonials',
    nav_messages: 'Messages',
    nav_settings: 'Settings',
    nav_contact: 'Contact',
    page_home: 'Home',
    page_blog: 'Blog',
    page_article: 'Article',
    page_notFound: 'Not Found',
    sections_title: 'Visible sections',
    sections_help: 'Hide entire site sections. Hidden sections stay reachable by direct URL but disappear from the menu.',
    nav_group_content: 'Content',
    nav_group_engagement: 'Engagement',
    nav_group_system: 'System',
    // Actions
    logout: 'Logout',
    login: 'Login',
    login_title: 'Admin Login',
    username: 'Username',
    password: 'Password',
    signIn: 'Sign In',
    signingIn: 'Signing in...',
    login_subtitle: 'Manage your portfolio content',
    backToSite: 'Back to site',
    add: 'Add',
    update: 'Update',
    edit: 'Edit',
    delete: 'Delete',
    cancel: 'Cancel',
    save: 'Save',
    // Skill categories
    cat_title: 'Title',
    cat_order: 'Order',
    cat_skillsCount: 'Skills',
    cat_noCategories: 'No categories yet. Add one to group your skills.',
    cat_none: '— None (legacy grouping) —',
    cat_manage: 'Manage categories',
    cat_faPlaceholder: 'Optional Persian category title',
    // Theme / language
    toggleTheme: 'Toggle theme',
    themeLight: 'Light mode',
    themeDark: 'Dark mode',
    toggleLang: 'Toggle language',
    menu: 'Menu',
    close: 'Close',
    // Dashboard
    dash_messages: 'Total Messages',
    dash_pending: 'Pending Testimonials',
    dash_views: 'Total Page Views',
    dash_topPages: 'Top Pages',
    dash_dailyTraffic: 'Daily Traffic (Last 30 Days)',
    dash_articles: 'Articles',
    dash_avg: 'Avg / day',
    dash_peak: 'Peak day',
    noData: 'No data yet',
    // Reports
    nav_reports: 'Reporting',
    nav_group_reports: 'Reports',
    rep_totalPageViews: 'Total Page Views',
    rep_articleViews: 'Article Views',
    rep_projectViews: 'Project Views',
    rep_articles: 'Articles',
    rep_projects: 'Projects',
    rep_likes: 'Total Likes',
    rep_topPages: 'Top Pages',
    rep_topLiked: 'Most Liked Articles',
    rep_topViewedArticles: 'Most Viewed Articles',
    rep_topViewedProjects: 'Most Viewed Projects',
    rep_views: 'views',
    rep_activeDays: 'Active Days',
    rep_peakDay: 'Peak Daily Views',
    rep_activityDetails: 'Daily Activity Details',
    rep_noActiveDays: 'No active days in the last 30 days.',
    // Articles
    art_language: 'Language',
    art_langEn: 'English',
    art_langFa: 'فارسی',
    art_slug: 'Slug',
    art_tags: 'Tags (comma-separated)',
    art_excerpt: 'Excerpt',
    art_content: 'Content',
    art_coverUrl: 'Cover URL',
    art_published: 'Published',
    art_readingTime: 'Estimated reading time: {n} min (auto-calculated on save)',
    art_read: 'Read',
    art_likes: 'Likes',
    // Videos
    vid_title: 'Title',
    vid_description: 'Description',
    vid_platform: 'Platform',
    vid_youtube: 'YouTube',
    vid_aparat: 'Aparat',
    vid_videoId: 'Video ID / Hash',
    vid_videoIdHelpYoutube: "YouTube video ID (the v= part of the URL).",
    vid_videoIdHelpAparat: 'Aparat video hash (from the share/embed URL).',
    vid_thumbnail: 'Thumbnail URL (optional)',
    vid_order: 'Order',
    vid_preview: 'Preview',
    // Projects
    proj_title: 'Title',
    proj_description: 'Description',
    proj_techStack: 'Tech stack',
    proj_techStackHelp: 'Comma-separated, e.g. Angular, NestJS, PostgreSQL',
    proj_liveUrl: 'Live URL (optional)',
    proj_repoUrl: 'Repository URL (optional)',
    proj_cover: 'Cover image (optional)',
    proj_order: 'Order',
    // Testimonials
    test_approve: 'Approve',
    test_reject: 'Reject',
    test_approved: 'Approved',
    test_rejected: 'Rejected',
    test_pending: 'Pending',
    filter_all: 'All',
    // Messages
    msg_read: 'Read',
    msg_unread: 'Unread',
    msg_markRead: 'Mark as Read',
    msg_subject: 'Subject',
    msg_noSubject: 'No subject',
    // Shared
    status: 'Status',
    published: 'Published',
    draft: 'Draft',
    actions: 'Actions',
    confirm_title: 'Confirm',
    confirm_delete: 'Are you sure you want to delete this item? This action cannot be undone.',
    confirm_yes: 'Delete',
    confirm_no: 'Cancel',
    toast_saved: 'Saved successfully',
    toast_added: 'Added successfully',
    toast_updated: 'Updated successfully',
    toast_deleted: 'Deleted',
    noItems: 'No items yet',
    toast_uploaded: 'Uploaded successfully',
    upload_failed: 'Upload failed',
    uploading: 'Uploading...',
    upload_drop: 'Drag & drop an image here, or click to browse',
    replace: 'Replace',
    choose_file: 'Choose file',
    remove: 'Remove',
    // About Me
    about_fullName: 'Full Name',
    about_title: 'Title',
    about_bio: 'Bio',
    about_avatarUrl: 'Avatar URL',
    about_resumeUrl: 'Résumé',
    about_resumeHint: 'PDF or DOC (max 10 MB)',
    about_email: 'Email',
    about_phone: 'Phone',
    about_location: 'Location',
    about_linkedin: 'LinkedIn',
    about_github: 'GitHub',
    about_youtube: 'YouTube',
    about_twitter: 'X / Twitter',
    about_instagram: 'Instagram',
    uploadedFile: 'Uploaded',
    uploadHint: 'Click or drag a file to upload',
    about_saved: 'Saved successfully!',
    // Educations
    edu_degree: 'Degree',
    edu_institution: 'Institution',
    edu_field: 'Field',
    edu_startDate: 'Start Date',
    edu_endDate: 'End Date',
    edu_description: 'Description',
    // Experiences
    exp_role: 'Role',
    exp_company: 'Company',
    exp_technologies: 'Technologies (comma separated)',
    exp_start: 'Start',
    // Skills
    skill_name: 'Name',
    skill_category: 'Category',
    skill_proficiency: 'Proficiency (0-100)',
    skills_display: 'Skill display',
    skills_display_help: 'Show skills as cards (hide proficiency bars) or with proficiency bars',
    theme_title: 'Theme',
    theme_help: 'Choose the site accent colors. Default keeps the original Iris Violet look.',
    theme_default: 'Default (Iris Violet)',
    theme_custom: 'Custom',
    theme_presets: 'Presets',
    theme_preset_iris: 'Iris Violet',
    theme_preset_crimson: 'Crimson Rose',
    theme_preset_azure: 'Azure',
    theme_preset_amber: 'Amber Glow',
    theme_primary: 'Primary color',
    theme_secondary: 'Secondary / gradient color',
    theme_preview: 'Preview',
    save_failed: 'Could not save changes',
    site_name: 'Site name',
    site_name_help: 'Shown in the page title, SEO tags and footer.',
    site_url: 'Site URL',
    site_url_help: 'Canonical domain, e.g. https://example.com',
  },
  fa: {
    appName: 'پنل ادمین',
    nav_dashboard: 'داشبورد',
    nav_about: 'درباره من',
    nav_experiences: 'تجربیات',
    nav_educations: 'تحصیلات',
    nav_skills: 'مهارت‌ها',
    nav_skill_categories: 'دسته‌بندی مهارت‌ها',
    nav_articles: 'مقالات',
    nav_videos: 'ویدیوها',
    nav_projects: 'پروژه‌ها',
    nav_testimonials: 'توصیه‌نامه‌ها',
    nav_messages: 'پیام‌ها',
    nav_settings: 'تنظیمات',
    nav_contact: 'تماس',
    page_home: 'خانه',
    page_blog: 'وبلاگ',
    page_article: 'مقاله',
    page_notFound: 'یافت نشد',
    sections_title: 'بخش‌های نمایشی',
    sections_help: 'پنهان کردن بخش‌های کامل سایت. بخش‌های پنهان از طریق نشانی مستقیم در دسترس می‌مانند اما از منو حذف می‌شوند.',
    nav_group_content: 'محتوا',
    nav_group_engagement: 'تعامل',
    nav_group_system: 'سیستم',
    logout: 'خروج',
    login: 'ورود',
    login_title: 'ورود به پنل ادمین',
    username: 'نام کاربری',
    password: 'گذرواژه',
    signIn: 'ورود',
    signingIn: 'در حال ورود...',
    login_subtitle: 'مدیریت محتوای نمونه‌کار',
    backToSite: 'بازگشت به سایت',
    add: 'افزودن',
    update: 'به‌روزرسانی',
    edit: 'ویرایش',
    delete: 'حذف',
    cancel: 'انصراف',
    save: 'ذخیره',
    cat_title: 'عنوان',
    cat_order: 'ترتیب',
    cat_skillsCount: 'مهارت‌ها',
    cat_noCategories: 'هنوز دسته‌بندی‌ای وجود ندارد. یکی اضافه کنید تا مهارت‌ها گروه‌بندی شوند.',
    cat_none: '— بدون دسته‌بندی (گروه‌بندی قدیمی) —',
    cat_manage: 'مدیریت دسته‌بندی‌ها',
    cat_faPlaceholder: 'عنوان فارسی دسته‌بندی (اختیاری)',
    toggleTheme: 'تغییر تم',
    themeLight: 'حالت روشن',
    themeDark: 'حالت تاریک',
    toggleLang: 'تغییر زبان',
    menu: 'منو',
    close: 'بستن',
    dash_messages: 'مجموع پیام‌ها',
    dash_pending: 'توصیه‌نامه‌های در انتظار',
    dash_views: 'مجموع بازدیدها',
    dash_topPages: 'پربازدیدترین صفحات',
    dash_dailyTraffic: 'ترافیک روزانه (۳۰ روز گذشته)',
    dash_articles: 'مقالات',
    dash_avg: 'میانگین / روز',
    dash_peak: 'پر‌بازدیدترین روز',
    noData: 'داده‌ای وجود ندارد',
    // Reports
    nav_reports: 'گزارش‌گیری',
    nav_group_reports: 'گزارش‌ها',
    rep_totalPageViews: 'مجموع بازدید صفحات',
    rep_articleViews: 'بازدید مقالات',
    rep_projectViews: 'بازدید پروژه‌ها',
    rep_articles: 'مقالات',
    rep_projects: 'پروژه‌ها',
    rep_likes: 'مجموع پسندها',
    rep_topPages: 'پربازدیدترین صفحات',
    rep_topLiked: 'بیشترین پسند مقالات',
    rep_topViewedArticles: 'بیشترین بازدید مقالات',
    rep_topViewedProjects: 'بیشترین بازدید پروژه‌ها',
    rep_views: 'بازدید',
    rep_activeDays: 'روزهای فعال',
    rep_peakDay: 'اوج بازدید روزانه',
    rep_activityDetails: 'جزئیات فعالیت روزانه',
    rep_noActiveDays: 'در ۳۰ روز گذشته روز فعالی ثبت نشده است.',
    art_language: 'زبان',
    art_langEn: 'انگلیسی',
    art_langFa: 'فارسی',
    art_slug: 'اسلاگ',
    art_tags: 'برچسب‌ها (با کاما جدا شود)',
    art_excerpt: 'خلاصه',
    art_content: 'محتوا',
    art_coverUrl: 'نشانی تصویر شاخص',
    art_published: 'منتشرشده',
    art_readingTime: 'زمان تقریبی مطالعه: {n} دقیقه (به‌طور خودکار محاسبه می‌شود)',
    art_read: 'زمان مطالعه',
    art_likes: 'پسندها',
    vid_title: 'عنوان',
    vid_description: 'توضیحات',
    vid_platform: 'پلتفرم',
    vid_youtube: 'یوتیوب',
    vid_aparat: 'آپارات',
    vid_videoId: 'شناسه ویدیو / هش',
    vid_videoIdHelpYoutube: 'شناسه ویدیوی یوتیوب (بخش v= آدرس).',
    vid_videoIdHelpAparat: 'هش ویدیوی آپارات (از آدرس اشتراک‌گذاری).',
    vid_thumbnail: 'نشانی تصویر کوچک (اختیاری)',
    vid_order: 'ترتیب',
    vid_preview: 'پیش‌نمایش',
    proj_title: 'عنوان',
    proj_description: 'توضیحات',
    proj_techStack: 'تکنولوژی‌ها',
    proj_techStackHelp: 'با کاما جدا کنید، مثلاً Angular، NestJS، PostgreSQL',
    proj_liveUrl: 'لینک نمایش زنده (اختیاری)',
    proj_repoUrl: 'لینک مخزن کد (اختیاری)',
    proj_cover: 'تصویر کاور (اختیاری)',
    proj_order: 'ترتیب',
    test_approve: 'تایید',
    test_reject: 'رد',
    test_approved: 'تایید‌شده',
    test_rejected: 'رد‌شده',
    test_pending: 'در انتظار',
    filter_all: 'همه',
    msg_read: 'خوانده‌شده',
    msg_unread: 'خوانده‌نشده',
    msg_markRead: 'علامت‌گذاری به عنوان خوانده‌شده',
    msg_subject: 'موضوع',
    msg_noSubject: 'بدون موضوع',
    status: 'وضعیت',
    published: 'منتشر‌شده',
    draft: 'پیش‌نویس',
    actions: 'عملیات',
    confirm_title: 'تایید',
    confirm_delete: 'آیا مطمئنید که می‌خواهید این مورد را حذف کنید؟ این عمل غیرقابل بازگشت است.',
    confirm_yes: 'حذف',
    confirm_no: 'انصراف',
    toast_saved: 'با موفقیت ذخیره شد',
    toast_added: 'با موفقیت افزوده شد',
    toast_updated: 'با موفقیت به‌روزرسانی شد',
    toast_deleted: 'حذف شد',
    noItems: 'هنوز موردی وجود ندارد',
    toast_uploaded: 'با موفقیت آپلود شد',
    upload_failed: 'آپلود ناموفق بود',
    uploading: 'در حال آپلود...',
    upload_drop: 'تصویر را اینجا رها کنید یا برای انتخاب کلیک کنید',
    replace: 'جایگزینی',
    choose_file: 'انتخاب فایل',
    remove: 'حذف',
    about_fullName: 'نام کامل',
    about_title: 'عنوان',
    about_bio: 'بیوگرافی',
    about_avatarUrl: 'نشانی آواتار',
    about_resumeUrl: 'رزومه',
    about_resumeHint: 'PDF یا DOC (حداکثر ۱۰ مگابایت)',
    about_email: 'ایمیل',
    about_phone: 'تلفن',
    about_location: 'موقعیت',
    about_linkedin: 'لینکدین',
    about_github: 'گیت‌هاب',
    about_youtube: 'یوتیوب',
    about_twitter: 'ایکس / توییتر',
    about_instagram: 'اینستاگرام',
    uploadedFile: 'آپلود شد',
    uploadHint: 'برای آپلود کلیک کنید یا فایل را بکشید',
    about_saved: 'با موفقیت ذخیره شد!',
    edu_degree: 'مدرک',
    edu_institution: 'موسسه',
    edu_field: 'رشته',
    edu_startDate: 'تاریخ شروع',
    edu_endDate: 'تاریخ پایان',
    edu_description: 'توضیحات',
    exp_role: 'سمت',
    exp_company: 'شرکت',
    exp_technologies: 'فناوری‌ها (با کاما جدا شود)',
    exp_start: 'شروع',
    skill_name: 'نام',
    skill_category: 'دسته‌بندی',
    skill_proficiency: 'تسلط (۰-۱۰۰)',
    skills_display: 'نمایش مهارت‌ها',
    skills_display_help: 'نمایش به صورت کارت (بدون نوار تسلط) یا با نوار تسلط',
    theme_title: 'تم',
    theme_help: 'رنگ‌های تاکید سایت را انتخاب کنید. حالت پیش‌فرض همان ظاهر بنفش ایریس است.',
    theme_default: 'پیش‌فرض (بنفش ایریس)',
    theme_custom: 'سفارشی',
    theme_presets: 'پیش‌فرض‌های آماده',
    theme_preset_iris: 'بنفش ایریس',
    theme_preset_crimson: 'سرخ یاسی',
    theme_preset_azure: 'لاجوردی',
    theme_preset_amber: 'کهربایی',
    theme_primary: 'رنگ اصلی',
    theme_secondary: 'رنگ ثانویه / گرادیان',
    theme_preview: 'پیش‌نمایش',
    save_failed: 'ذخیره تغییرات ناموفق بود',
    site_name: 'نام سایت',
    site_name_help: 'در عنوان صفحه، متاداده‌های سئو و فوتر نمایش داده می‌شود.',
    site_url: 'آدرس سایت',
    site_url_help: 'دامنه اصلی، مثلاً https://example.com',
  },
};

@Injectable({ providedIn: 'root' })
export class AdminI18nService {
  private readonly storageKey = 'admin_lang';

  currentLang = signal<Language>('fa');

  constructor() {
    const stored = localStorage.getItem(this.storageKey) as Language | null;
    if (stored === 'en' || stored === 'fa') {
      this.currentLang.set(stored);
    }
    this.applyLang(this.currentLang());
    effect(() => this.applyLang(this.currentLang()));
  }

  t(key: string, params?: Record<string, string | number>): string {
    let str = TRANSLATIONS[this.currentLang()][key] ?? key;
    if (params) {
      for (const [k, v] of Object.entries(params)) {
        str = str.replace(new RegExp(`\\{${k}\\}`, 'g'), String(v));
      }
    }
    return str;
  }

  get isFa(): boolean {
    return this.currentLang() === 'fa';
  }

  toggleLang(): void {
    const next: Language = this.currentLang() === 'en' ? 'fa' : 'en';
    this.currentLang.set(next);
    localStorage.setItem(this.storageKey, next);
  }

  private applyLang(lang: Language): void {
    const html = document.documentElement;
    html.lang = lang;
    html.dir = lang === 'fa' ? 'rtl' : 'ltr';
  }
}
